...
# M1: canonical interfaces via their concrete spec modules
from spec.bot_core import BotCore
from spec.skills import SkillRegistry
from spec.llm import PlannerModel, CriticModel
from spec.experience import ExperienceRecorder
...
class DummyBotCore(BotCore):
    ...
    def get_world_state(self) -> WorldState:  # type: ignore[override]
        ...
        return WorldState(
            ...
            tech_state={},                              # unknown tech state (Phase 0)
...
class SimpleAgentLoop:
    ...
    This is **not** the big M8 AgentLoop; it's just a stub matching
    what tests expect: set_goal / step / get_status.
    """
...
def create_phase0_agent(env: EnvProfile) -> tuple[BotCore, SimpleAgentLoop]:
