...
#!/usr/bin/env python3
"""
Smoke test for LLMStack.

Usage:
    python3 scripts/smoke_llm_stack.py
    python3 scripts/smoke_llm_stack.py --perf
"""
...
def run_smoke(perf: bool = False) -> int:
    """
    Run a planning smoke test against the current environment profile.

    If perf=True, prints a perf-oriented timing line so you can compare runs,
    alongside llama.cpp's own llama_perf_context_print output.
    """
    # -----------------------------------------------------------------------
    ...
    if perf:
        print(f"\n[perf] wall-clock time: {elapsed:.3f} s")
        print(
            "[perf] note: llama-cpp per-token timings will also appear above as "
            "llama_perf_context_print if enabled."
        )
    else:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Smoke test for LLMStack.")
    parser.add_argument(
        "--perf",
        action="store_true",
        help="run in perf mode (prints timing info)",
    )
    args = parser.parse_args()
    return run_smoke(perf=args.perf)
